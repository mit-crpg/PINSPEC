...
for i in range(len(E0)):  # for each resonance
	...
	y = (((x + 1j) / 2) * squig[i])
        #print y
        G=spec.wofz(y)
	#numpy.savetxt('breakG'+str(i), G.real, newline='\n', delimiter='  ')
        #troubleshooting
	#EXIST=(G)
	#numpy.savetxt('breakG'+str(i), EXIST, newline='\n', delimiter='  ')
	psichi = (math.pi * squig[i] / (2 * (math.pi ** 0.5))) * G
	#numpy.savetxt('breakPsichi'+str(i), psichi, newline='\n', delimiter='  ')
 	#print psichi
	psi = psichi.real
        #print psi
	chi = psichi.imag
	...
	#numpy.savetxt('breaksigma_g'+str(i), sigma_g, newline='\n', delimiter='  ')        
	#if sigma_g.any>1e+100:
		#print "Large number on iteration: "+str(i)
                #numpy.savetxt('breakpsi'+str(i), psi, newline='\n', delimiter='  ')
                #print "GN=: "+str(GN[i])+"\n"+"Gamma=: "+str(Gamma[i])+"\n"+"GG=: "+str(GG[i])+"\n"+"E0=: "+str(E0[i])+"\n"+"r=: "+str(r[i])+"\n"
...
# write output file
out_name = cur_dir + "/../XS_Lib/U-238-capturefake.txt"
...
#-----------------
        ...
        #troubleshooting
	#EXIST=(x)
	#EXIST=numpy.transpose(EXIST)
	#numpy.savetxt('breakx'+str(i), EXIST, newline='\n', delimiter='  ')
# Clear arrays
del sigma_g
del E
del sigma_n
